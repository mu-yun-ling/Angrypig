...
playing::playing(QWidget *parent) : QWidget{parent}
{
    ...
    //还有一个保险措施
    safe=new bird(":/birds/bird_red2.png",":/birds/bird_red2.png",2 );
    safe->setPos(200,200);
...
void playing::Add_slingshot(pig*ptr)
{
    //弹弓图
...
void playing::set_pig(pig*ptr)//放置pig
{
...
void playing:: GameOver()
{
    ...
    QTimer::singleShot(4000, this, [this]() {
        ...
        textItem->setDefaultTextColor(QColor(43, 141, 167));
        textItem->setPos(
            (Scene->width() - textItem->boundingRect().width()) / 2+50, (Scene->height() - textItem->boundingRect().height()) / 2
            );
        textItem->setZValue(51);
...
void playing::boom(bird*ptr)
{
    ...
    ptr->setPixmap(explode);

    QTimer::singleShot(300,this,[this, ptr]()
                       {
